...
def norm(edge_index, edge_weight=None, num_nodes=None, improved=False,
    ...
    deg = scatter_add(torch.abs(edge_weight), col, dim=0, dim_size=num_nodes)
...
class FAConv(MessagePassing):
    ...
    def __init__(self, channels, eps: float = 0.1, dropout: float = 0.0,
    ...
    def reset_parameters(self):
        torch.nn.init.xavier_normal_(self.att_l.weight, gain=1.414)
        torch.nn.init.xavier_normal_(self.att_r.weight, gain=1.414)
        # self.att_l.reset_parameters()
        # self.att_r.reset_parameters()
        self._cached_edge_index = None
        self._cached_adj_t = None

    def forward(self, x, x_0, edge_index, edge_weight = None, return_attention_weights=None):
        """
        Args:
        ...
        if isinstance(edge_index, Tensor):
            ...
            if cache is None:
                edge_index, edge_weight = norm(  # yapf: disable
    ...
    def message(self, x_j, alpha_j, alpha_i, edge_weight):
