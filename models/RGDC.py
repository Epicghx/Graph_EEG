...
class GraphConvolution(SGConv):
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        if not self.cached or self.cached_result is None:
            ...
            #     return self.lin(self.cached_result) + self.bias
            # else:
...
class RGDC(nn.Module):
    ...
    def forward(self, data, training=True, samp_type='rel_ber'):
        ...
        adj = self.norm(edge_weight).reshape(-1)  # adj normalization
        ...
        for i in range(self.nlay):
            ...
            if i == 0:
                # mask_mat = mask_vec[:self.num_edges]
                # adj_lay = (mask_mat * adj).repeat(batch_size)
                ...
                # for j in range(self.nblock):
                #     mask_mat = mask_vec[j * self.num_edges:(j + 1) * self.num_edges]
                #     adj_lay = (mask_mat * adj).repeat(batch_size)
                #
                #     if i < (self.nlay - 1):
                #         if j == 0:
        ...
        # output = out.reshape(batch_size, -1) # B x (C x F) 62*256
