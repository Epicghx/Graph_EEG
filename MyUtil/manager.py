    ...
    def train(self, optimizer):
        ...
        for batch_idx, data in enumerate(self.train_loader, 0):
            ...
            if 'GDC' in self.args.arch:
                ...
                tot_loss = KL_loss + kld_loss
                # loss = tot_loss + self.args.weight_decay * l2_reg + self.alpha * self.model.edge_weight.norm(1)+ self.regu(self.model)
                ...
                KL_loss = F.kl_div(F.log_softmax(output, dim=1), F.softmax(labelsprob, dim=1),
                                   reduction='sum')  # good one
                loss = KL_loss + self.alpha * self.model.edge_weight.norm(1) + self.regu(self.model)
                loss.backward()
    ...
    def eval(self, biases=None):
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(self.test_loader):
                ...
                if 'GDC' in self.args.arch:
                    output, _, _ = self.model(inputs, training=True)
                elif self.args.arch == 'RGNN':
                    output, _ = self.model(inputs)
                # outs = np.stack(outs)
                # ipdb.set_trace()
