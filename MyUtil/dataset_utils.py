...
class dataset_heterophily(InMemoryDataset):
    def __init__(self, root='D:/tmp/', name=None,
                 p2raw=None,
                 train_percent=0.01,
...
def DataLoader(name):
    ...
    if name in ['cora', 'citeseer', 'pubmed']:
        ...
        dataset = Planetoid(root_path, name, transform=T.NormalizeFeatures())
        ...
        dataset = Actor(
            root=root_path+'Actor', transform=T.NormalizeFeatures())
    elif name in ['texas', 'cornell', 'wisconsin']:
        dataset = WebKB(root=root_path,
